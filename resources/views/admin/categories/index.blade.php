<x-layouts.base>
    ...
    <div class="container px-2 py-5 mx-auto max-w-7xl">
        <div class="flex justify-between py-8">
            <h3 class="font-bold text-2xl">Manage Categories</h3>
            <div>
                <a href="{{ route('admin.categories.create') }}" class="inline-flex items-center justify-center px-4 py-2 text-base font-medium leading-6 text-white whitespace-no-wrap bg-rose-500 border border-transparent rounded-md shadow-sm hover:bg-rose-600 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-rose-600">Add Category</a>
            </div>
        </div>
                    ...
                    <div class="overflow-hidden border rounded-lg">
                        <table class="min-w-full divide-y divide-neutral-200">
                            ...
                            <tr class="text-neutral-900 bg-gray-100">
                                ...
                                <th class="px-5 py-4 text-sm font-bolder text-left uppercase">Slug</th>
                                <th class="px-5 py-4 text-sm font-bolder text-center uppercase">Count</th>
                            ...
                            @foreach($categories as $category)
                                <tr class="text-neutral-800 hover:bg-gray-100">
                                    ...
                                    <td class="px-5 py-4 text-sm whitespace-nowrap">{{ $category->slug }}</td>
                                    <td class="px-5 py-4 text-sm whitespace-nowrap text-center">{{ $category->courses->count() }}</td>
                                    <td class="px-5 py-4 text-sm font-medium text-right whitespace-nowrap space-x-4">
                                        @can('update', $category)
                                            <a class="text-rose-500 hover:text-rose-600"
                                               href="{{ route('admin.categories.edit', compact('category')) }}">Edit</a>
                                        @endcan
                                        @can('delete', $category)

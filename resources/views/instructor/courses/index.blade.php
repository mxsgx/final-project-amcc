        ...
        <div class="flex justify-between py-8">
            ...
            <div>
                <a href="{{ route('instructor.courses.index') }}"
                   class="inline-flex items-center justify-center px-4 py-2 text-base font-medium leading-6 text-white whitespace-no-wrap bg-rose-500 border border-transparent rounded-md shadow-sm hover:bg-rose-600 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-rose-600">Add
                    Course</a>
            </div>
        </div>
                    ...
                    <div class="overflow-hidden border rounded-lg">
                        <table class="min-w-full divide-y divide-neutral-200">
                            ...
                            <tr class="text-neutral-900 bg-gray-100">
                                <th class="px-5 py-4 text-sm font-bolder text-left uppercase">Title</th>
                                <th class="px-5 py-4 text-sm font-bolder text-center uppercase">Lectures</th>
                                <th class="px-5 py-4 text-sm font-bolder text-center uppercase">Date</th>
                            ...
                            @foreach($courses as $course)
                                <tr class="text-neutral-800 hover:bg-gray-100">
                                    ...
                                    <td class="px-5 py-4 text-sm text-center whitespace-nowrap">{{ $course->lectures->count() }}</td>
                                    <td class="px-5 py-4 text-sm text-center whitespace-nowrap">{{ $course->created_at->format('d F, Y') }}</td>
                                    <td class="px-5 py-4 text-sm font-medium text-right whitespace-nowrap space-x-4">
                                        @can('update', $course)
                                            <a class="text-rose-500 hover:text-rose-600"
                                               href="{{ route('instructor.courses.edit', compact('course')) }}">Edit</a>
                                        @endcan
                                        @can('delete', $course)
                                            <div x-data="{ modalOpen: false }"
                                                ...
                                                <template x-teleport="body">
                                                    <form action="{{ route('instructor.courses.delete', compact('course')) }}"
                                                        ...
                                                        <div x-show="modalOpen"
                                                            ...
                                                            <div class="relative w-auto pb-8">
                                                                @csrf
                                                                @method('DELETE')
                                                                <p>Are you sure want to delete this course? Any relation
                                                                    to
